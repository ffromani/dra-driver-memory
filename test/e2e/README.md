...
The E2E tests want to manage the Memory allocation on the tested nodes.
The Memory allocation state is ultimately shared node state. Thus, the test should run serially
...
- `DRAMEM_E2E_TEST_IMAGE`: (mandatory) the full pullSpec of the test image the suite should
  use as container image to run test containers. The default CI configuration sets this
  value automatically.
- `DRAMEM_E2E_TARGET_NODE`: (optional) the memory-related tests don't need any specific node.
